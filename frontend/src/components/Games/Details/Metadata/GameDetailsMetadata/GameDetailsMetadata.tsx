...
import { ReShadeDetectionStatus } from "@bindings/github.com/phergul/fiach/internal/services/dto/models";
import type {
  UseGameOptiScalerResult,
  UseGameReShadeDetectionResult,
  UseGameReShadeResult,
} from "@hooks";
...
const formatReShadeDetectionStatus = (
  reShadeDetection: UseGameReShadeDetectionResult,
) => {
...
const formatReShadeStatus = (
  reShade: UseGameReShadeResult,
  fallback: UseGameReShadeDetectionResult,
) => {
...
export const GameDetailsMetadata = ({
  ...
  const metadataItems = [
    { Icon: Package, label: "Mods installed", value: String(modCount) },
    {
      Icon: Database,
      label: "Storage used",
      value: formatStorageUsage(storageUsedBytes, isStorageUsageLoading),
    },
    ...
    metadataItems.push(
      {
        Icon: Sparkles,
        label: "ReShade",
        value: formatReShadeStatus(reShade, reShadeDetection),
      },
      {
        Icon: Gauge,
        label: "OptiScaler",
        value: formatOptiScalerStatus(optiScaler),
      },
    );
  }
      ...
      {metadataItems.map((item) => (
        <div className="game-details-metadata-item" key={item.label}>
          <dt className="game-details-metadata-label">
            <item.Icon
              className="game-details-metadata-icon"
              aria-hidden="true"
            />
