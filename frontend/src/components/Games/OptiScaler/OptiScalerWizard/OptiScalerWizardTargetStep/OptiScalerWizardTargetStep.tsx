  ...
  return (
    <div className="optiscaler-wizard-content">
      <dl className="optiscaler-wizard-summary">
        ...
        <div>
          <dt>Directory</dt>
          <dd>{targetRelativePath}</dd>
        </div>
        <div>
          <dt>Architecture</dt>
          <dd>{selection.candidate?.architecture ?? 'x64'}</dd>
        </div>
        <div>
          <dt>Action</dt>
          <dd>{actionLabel}</dd>
      ...
      {selection.candidate?.hasOptiScaler && (
        <p className="optiscaler-wizard-warning">
          OptiScaler files are already present in this directory. Adoption will have these files
          become managed.
        </p>
      )}
    </div>
